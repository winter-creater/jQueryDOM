  ...
  return {
    ...
    find: function find(selector) {
      ...
      for (var i = 0; i < elements.length; i++) {
        ...
        array = array.concat(elements2); // elements[i].querySelectorAll(selector)伪数组，Array.from()转化为真数组
    ...
    children: function children() {
      var array = [];
      this.each(function (node) {
        array.push.apply(array, _toConsumableArray(node.children)); //展开操作符，相当于 array.push(node.children[0],node.children[1],node.children[2]...)
      });
      return jQuery(array);
    },
